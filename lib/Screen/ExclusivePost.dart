...
class ExclusivePost extends StatefulWidget {

  const ExclusivePost({Key? key}) : super(key: key);

  @override
  State<ExclusivePost> createState() => _ExclusivePostState();
}

class _ExclusivePostState extends State<ExclusivePost> {
  ...
  void _loadMore() async {
    if (_hasNextPage == true &&
      ...
      try {
        await Provider.of<Posts>(context, listen: false).exclusivePosts(page: _page).then((value){
          setState(() {
            postData=Provider.of<Posts>(context, listen: false).exclusiveposts;
  ...
  Widget build(BuildContext context) {
    ...
    // final postData=Provider.of<Posts>(context, listen: false).exclusiveposts;
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title:Text("Exclusive",style:  FormaDJRDisplayBold.copyWith(
      ...
      bottomNavigationBar: BottomBar(pageName: "Exclusive Post",),
    );
  }
}
