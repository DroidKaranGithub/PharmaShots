...
class SpotlightPost extends StatefulWidget {

  const SpotlightPost({Key? key}) : super(key: key);

  @override
  State<SpotlightPost> createState() => _SpotlightPostState();
}

class _SpotlightPostState extends State<SpotlightPost> {
  ...
  void _loadMore() async {
    if (_hasNextPage == true &&
      ...
      try {
        await Provider.of<Posts>(context, listen: false).spotlightPosts(page: _page).then((value){
          setState(() {
            postData=Provider.of<Posts>(context, listen: false).spotlightposts;
  ...
  Widget build(BuildContext context) {
    ...
    // final postData=Provider.of<Posts>(context, listen: false).spotlightposts;
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Spotlight",style:  FormaDJRDisplayBold.copyWith(
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder:(context,index){
                return postlistwidget(postData[index],read_mode: "web");
              },
      ...
      bottomNavigationBar: BottomBar(pageName: "Spotlight Post",),
    );
  }
}
